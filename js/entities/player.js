...
game.PlayerEntity = me.ObjectEntity.extend({
	...
	init: function(x, y, settings) {
		...
		// network player?
		this.network = settings.network || false;
		...
		// update the network occasionally
		this.updateNetworkThrottled = throttle( 250, true, (this.updateNetwork).bind(this) );
	...
	update: function() {
	    ...
	    // if the velocity changed then update the network
	    if ( ! this.network && ! this.vel.equals( this.oldVel ) ) {
	    	this.updateNetwork();
		}

		// update animation if necessary
		if ( this.vel.x !== 0 || this.vel.y !== 0 ) {
			// update the network of your position
			if ( ! this.network ) {
				this.updateNetworkThrottled();
			}
